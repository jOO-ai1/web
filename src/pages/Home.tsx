...
export default function Home() {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      ...
      >
        <h1 className="text-4xl md:text-6xl font-bold mb-6 text-[#111] font-montserrat tracking-wider">
          {t("slogan")}
        </h1>
        ...
        {collections.map((col, index) => (
          ...
          >
            <Link to={`/collections/${col.id}`}>
              <div className="glass rounded-2xl shadow-lg overflow-hidden h-80 flex flex-col justify-end relative group">
                <img 
                  src={col.image} 
                  alt={col.name[lang]} 
                ...
                <div className="relative z-10 p-6">
                  ...
                  <div className="text-gray-200">{col.desc[lang]}</div>
                </div>
              </div>
            </Link>
          </motion.div>
        ...
        {products.slice(0, 3).map((prod, index) => (
          ...
          >
            <Link to={`/product/${prod.id}`}>
              <div className="glass p-6 rounded-2xl shadow-xl border border-white/25 group hover:shadow-2xl transition-all duration-300">
